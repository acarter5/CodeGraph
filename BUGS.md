...
- [x] 🔴 **#6 — Graph output is computed then discarded** ✅ FIXED
  ...
  **✅ DONE — call-site anchors implemented.** `edges[].callSiteRect` (normalized [0..1] within the caller's image) is now populated by measuring each call token's `getBoundingClientRect()` against the captured `#content` box in the webview at capture time — exactly the "don't reconstruct pixels from font metrics" plan above (normalizing instead of multiplying by `scale`, which cancels out). Flow: `Scanner.callExpressionNames` → `MapNode.callSites` (built pre-`loadPage`, `calleeId` linked post-resolution) → webview `measureCallSites` → `View` writes rects back → `serializeGraph` emits one edge per call site with `callSiteRect` → client anchors the connector start there. See CLAUDE.md "Call-site-anchored connectors". The old logical-position `callSite` field was dropped in favor of `callSiteRect`. (Off-by-one #7 was a prerequisite and is fixed.)
