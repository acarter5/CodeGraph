...
extension.ts            registers command, resolves the entry definition
  └─ Builder            buildNodeMap() recursion + buildNodeGraph() tree
       ...
       ├─ Parser        parses the full file → ts-morph AST, looks the function
       │                node up by position (definition range) — already positioned
       ├─ Scanner       takes that positioned node, collects CallExpression locations
...
- `client/src/PluginUI.svelte` (UI iframe) — URL input (paste the clipboard URL the extension copies). On "Render graph" it `fetch`es `graph.json`, then each PNG (resolved by `image.file` next to the manifest), and posts `{ manifest, images: [{definitionId, tiles: ImageTile[] | null}] }` to the sandbox. The iframe does this because the sandbox has no `fetch`. It **slices each PNG into a grid of ≤4096px tiles through a canvas** (`fitImageTiles`) before handing bytes to the sandbox — this is load-bearing, not just for tiling:
  - Figma `createImage` **accepts the original dom-to-image PNGs (returns a hash) but renders them blank**; round-tripping through `<img>` → canvas → `toBlob` yields a normalized PNG it renders reliably. So we always re-encode every tile through canvas, even a single one already under the limit. (Confirmed empirically: a re-encoded 4096px image renders; an un-re-encoded 3363px original renders blank.)
  ...
  - **Tiling (the big-function fix):** a snapshot larger than **4096px** (Figma's hard `createImage` limit; long functions at `scale: 3` exceed it) is sliced into a grid of tiles each ≤4096px in both dims (`Math.ceil(dim/4096)` even splits). Each `ImageTile` carries its normalized `[0..1]` position/size within the full image; the sandbox lays one image-filled rect per tile so they abut exactly and fill the same logical box — at **full capture resolution, no downscaling**. This replaced the old whole-image downscale (which made long functions unreadable — blurry, sometimes <1 device-px per logical px). The single-column case (width ≤4096) is just N vertical strips.
  - If a PNG can't be decoded/sliced at all, the image is sent as `tiles: null` and the sandbox draws a red **"⚠ Image too large to render"** box — loud, never a silent blank. (The sandbox is all-or-nothing per def: if any tile's `createImage` throws, the whole def falls back to that box rather than rendering a partial snapshot.)
...
>
> **Known rough edges / next:** large snapshots are tiled into ≤4096px grid cells at full resolution (see above); a call-site connector's start is an absolute `position`, so it won't follow a box that's manually moved.
...
- **Node matching:** the `Parser` looks the function node up **by position** in the full-file AST — `getPositionOfLineAndCharacter(definition range)` → the widest function-like node whose span falls inside the range, with an ancestor-walk fallback for name-only ranges (and the variable-declaration initializer for `const foo = () => …`). The node is already positioned, so there's no structural re-match. (This replaced the old isolated-snippet reparse + `__CodeGraphWrapper__` fallback + `Scanner._findPositionedFunctionNode`/`looksLike` match, which was fragile on large functions — a miss became a `positionFail` that dropped the whole subtree. `looksLike` in `utils/tsMorph.ts` is now unused by the pipeline.)
